...
def aantal_kluizen_vrij():
    ...
    """
    kluisbestand = open('fa_kluizen.txt', 'r')              # het bestand fa_kluizen.txt openen.
    kluisregels = kluisbestand.readlines()                  # hier leest de regel in de bestand
    kluisbestand.close()                                     # het bestand fa_kluizen.txt sluiten.
    kluizen = len(kluisregels)                              # om de lengte van de kluisregel te weten.
    vrijkluizen = 12 - kluizen
    return vrijkluizen
...
def nieuwe_kluis():
    ...
    bestand.close()
    kluisnr = ['1', '2', '3', '4', '6', '7', '8', '9', '10', '11', '12']
    for y in regels:
        m = y.split(';')
        if kluis[0] in kluisnr:
            kluisnr.remove(kluis[0])
        ...
        if ';' in kluiscode:
            ...
            if kluisnr > 0:
                kluiscode = input('schrijf je code')

    else:
        print('voe je nieuwe code \n')
...
def kluis_openen():
    ...
    resultaat = kluisnummer + ';' + kluiscode
    y = open('fa_kluizen.txt')
    regels = y.readlines()
    y.close()
    for y in regels:
        if resultaat == y.replace("\n",""):
            return True
...
def kluis_teruggeven():
    ...
    """
    kluisnummer = input('voer je kluisnummer in')  #  om de gebruiker kluisnummer intevoeren
    kluiscode = input('voer je kluiscode in')  # om de gebruiker kluiscode intevoeren
    samenvatting = kluisnummer + ';' + kluiscode  # schrijf kluisnummer en kluiscode met ; tussen
    bestand = open('fa_kluizen.txt')  # hier open de bestand om all regles te zien
    regels = bestand.readlines()  # hier leest all regels in het bestand on gaaat hun in gewoon lijst opslaan
    samenvatting.close()  #  hier gaat de sluit bestand

    bestand = open('fa_kluizen.txt', "w")  # hier open de bestan om te schrijven

    for y in regels:
        if samenvatting != y.replace("\n", ""):  #
            bestand.write(y)  #
...
def development_code():
    ...
    if keuze == 1:
        ...
        print(f"er zijn {aantal_vrij} beschikbaar")
